...
function HistoryPage({ onHome, onHistory, onCalendar, onAnalysis, onWishlist, onChat, onAddSpend, current }) {
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            for (let j = 0; j < 7; j++) {
                if (i === 0 && j < firstDay) {
                    week.push({ date: prevMonthLastDate - (firstDay - j - 1), isCurrentMonth: false })
                } else if (currentDay > lastDate) {
                    week.push({ date: nextDay++, isCurrentMonth: false })
                } else {
                    week.push({ date: currentDay++, isCurrentMonth: true })
                }
            }
            grid.push(week)
    ...
    useEffect(() => {
        ...
        if (isThisMonth) {
            ...
            const weekIdx = fullDatesGrid.findIndex(week =>
                week.some(d => d.date === targetDate && d.isCurrentMonth)
            )
    ...
    }, [fullDatesGrid, year, month, today])
    ...
    const handleScroll = useCallback(() => {
        ...
        if (closestDateStr) {
            ...
            const weekIdx = fullDatesGrid.findIndex(week =>
                week.some(day => day.date === d && day.isCurrentMonth)
            )
            if (weekIdx !== -1) {
                setActiveWeekIndex(prev => prev !== weekIdx ? weekIdx : prev)
            }
        }
    }, [fullDatesGrid])
    ...
    return (
        <div style={{
            ...
            maxWidth: "360px", margin: "0 auto", overscrollBehavior: "none",
            ...
            <div
                ref={scrollRef}
                style={{ flex: 1, overflowY: "auto", overflowX: "hidden", position: "relative", fontFamily: "'GriounPolice', cursive" }}
            >
                ...
                <div style={{
                    ...
                    {["일", "월", "화", "수", "목", "금", "토"].map((day, i) => (
                        <div key={day} style={{ textAlign: "center", fontSize: "11px", color: i === 0 ? "#F4A7B9" : i === 6 ? "#7F77DD" : "#bbb", boxSizing: "border-box" }}>
                            {day}
                        </div>
                    ))}
                </div>
                            ...
                            {week.map((d, j) => {
                                ...
                                return (
                                    <div key={`${i}-${j}`} onClick={() => d.isCurrentMonth && handleDateClick(d.date)}
                                         style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "4px 2px", borderRadius: "8px", cursor: d.isCurrentMonth ? "pointer" : "default", boxSizing: "border-box" }}>
                                        <div style={{ width: "26px", height: "26px", borderRadius: "50%", background: isSelected ? "#F4A7B9" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "13px", fontWeight: isSelected || isToday ? "700" : "400", color: isSelected ? "white" : isToday ? "#F4A7B9" : !d.isCurrentMonth ? "#ddd" : j === 0 ? "#F4A7B9" : j === 6 ? "#7F77DD" : "#333", marginBottom: "2px" }}>
                                            {d.date}
                                        </div>
                                        {income > 0 && <div style={{ fontSize: "8px", color: "#7F77DD", lineHeight: 1.1 }}>+{income >= 10000 ? `${Math.floor(income / 1000)}k` : income}</div>}
                                        {expense > 0 && <div style={{ fontSize: "8px", color: "#F4A7B9", lineHeight: 1.1 }}>-{expense >= 10000 ? `${Math.floor(expense / 1000)}k` : expense}</div>}
                                    </div>
                                )
                            })}
                        </div>
                        ...
                        grouped.map(([dateStr, items]) => (
                            <div key={dateStr} ref={el => {
                                if (el) {
                                    ...
                                    delete dateRefs.current[dateStr]
                                }
                            }} style={{ marginBottom: "20px" }}>
                                <p style={{ margin: "0 0 8px", fontSize: "13px", color: "#999", fontWeight: "600" }}>
                                    {getDayLabel(dateStr)}
                                </p>
                                {items.map(item => (
                                    <div key={item.id} style={{ background: "white", borderRadius: "14px", padding: "13px 14px", marginBottom: "8px", boxShadow: "0 1px 6px rgba(0,0,0,0.05)", display: "flex", alignItems: "center", gap: "12px" }}>
                                        ...
                                        <div style={{ display: "flex", alignItems: "center", gap: "8px", flexShrink: 0 }}>
                                            ...
                                            <button onClick={() => handleDelete(item.id)} style={{ background: "none", border: "none", fontSize: "14px", cursor: "pointer", color: "#ddd", padding: 0 }}>🗑️</button>
                                        </div>
                                    </div>
                                ))}
                            </div>
