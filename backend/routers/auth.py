...
def signup(request: SignupRequest, db: Session = Depends(get_db)):
    ...
    return {"message": "회원가입 성공!", "token": token, "nickname": new_user.nickname, "user_id": new_user.id}
...
def login(request: LoginRequest, db: Session = Depends(get_db)):
    ...
    return {"message": "로그인 성공!", "token": token, "nickname": user.nickname, "user_id": user.id}